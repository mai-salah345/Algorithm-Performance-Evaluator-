...
class AlgorithmEvaluatorApp(tk.Tk):
    ...
    def _run_auto(self, code):
        ...
        if "O(2ⁿ)" in algo_name or "O(2^n)" in algo_name:
            custom_sizes = [5, 8, 10, 12, 14, 16, 18, 20]
        elif "O(n³)" in algo_name or "O(n^3)" in algo_name:
            custom_sizes = [10, 20, 40, 80, 100, 120, 150]
        elif "O(n²)" in algo_name or "O(n^2)" in algo_name:
            custom_sizes = [10, 50, 100, 200, 400, 600, 800]
        elif "O(n)" in algo_name or "O(n log n)" in algo_name:
            # هنا الـ Linear والـ Merge Sort هياخدوا الأرقام الضخمة عشان نفرق بينهم
            custom_sizes = [1000, 3000, 5000, 8000, 12000, 16000, 20000]
            rep = 5  
        else:
            # هنا الـ O(1) هتاخد مصفوفات صغيرة عشان نتجنب ضوضاء الميموري
            custom_sizes = [10, 50, 100, 200, 400, 600, 1000]
            rep = 5
